...
const CategoryNavMobile = ({ setCatnavMobile }) => {
  ...
  return (
    <div className='w-[full] md:w-[40%] h-full backDrop p-8 pr-0 pt-0'>
      ...
      <div className='flex flex-col uppercase font-semibold text-xl gap-y-6'>
        {data?.map(category => (
          <Link to={`products/${category.id}`} key={category.id} className='hover:scale-105 translate-x-4 duration-500 cursor-pointer uppercase hover:text-accent hover:underline w-[100px]'>
